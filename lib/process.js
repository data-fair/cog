...
module.exports = async (processingConfig, tmpDir, axios, log, token) => {
  ...
  const communes = await fs.readJsonSync(path.join(tmpDir, 'communes.json'))
  for (const commune of communes.filter((commune) => commune.code.startsWith(processingConfig.filter))) {
